...
pub fn render_bar(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let (row1, row2) = match app.menu {
        ...
        MenuState::Top { cursor } => {
            let r1 = top_spans(items, cursor, theme);
            let r2 = sub_name_spans(items[cursor].sub);
            (r1, r2)
        }

        MenuState::Sub { top, cursor } => {
            let r1 = sub_item_spans(items[top].sub, cursor, theme);
        ...
        MenuState::SubSub { top, sub, cursor } => {
            ...
            let r1 = subsub_item_spans(children, cursor, theme);
...
fn top_spans(items: &'static [TopItem], cursor: usize, theme: &Theme) -> Vec<Span<'static>> {
...
fn sub_item_spans(sub: &'static [SubItem], cursor: usize, theme: &Theme) -> Vec<Span<'static>> {
    let mut spans = Vec::new();
    spans.push(Span::raw(" "));
    for (i, item) in sub.iter().enumerate() {
        if i > 0 {
...
fn subsub_item_spans(children: &'static [SubSubItem], cursor: usize, theme: &Theme) -> Vec<Span<'static>> {
    let mut spans = Vec::new();
    spans.push(Span::raw(" "));
