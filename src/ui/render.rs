...
const FKEY_BAR: &str =
    "F1=Help  F2=Edit  F5=Note  F6=Props  F7=Mark  F8=VwMgr  F10=Menu";

const SECTION_PREFIX: &str = "  ";
const ITEM_PREFIX:    &str = "   \u{2022} ";

/// Split `buffer` into (left, highlighted, right) around the char at `cursor`.
/// `highlighted` is the character at cursor position, or a space if cursor is
/// past the end, so the cursor is always visible as a reversed cell.
fn cursor_split(buffer: &str, cursor: usize) -> (String, String, String) {
    let mut chars = buffer.chars();
    let left:  String = chars.by_ref().take(cursor).collect();
    let hi_char = chars.next();
    let right: String = chars.collect();
    let highlighted = match hi_char {
        Some(c) => c.to_string(),
        None    => " ".to_string(),
    };
    (left, highlighted, right)
}
